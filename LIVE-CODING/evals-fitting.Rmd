...
Write code to make a histogram of `score`. Comment on the shape. Also calculate numerical summaries.

### Exercise 2

Write code to make a histogram of `bty_avg`. Comment on the shape. Also calculate numerical summaries.
...
```{r}
evals_slr <- lm(score ~ bty_avg, data = evals)
...
We know from Part I that `bty_avg` has a weak relationship with the course evaluation `score`.  Does `gender` influence `score`? Make a boxplot to examine this. Also calculate numerical summaries of the score by gender.
...
Create a scatter plot of `score` vs `bty_avg` colored by `gender`. Use `geom_smooth` to fit two lines to the data: one for males and another for females.
...
Fit a main effects linear model, `evals_main`, predicting course evaluation `score` from average beauty rating (`bty_avg`) and `gender`. 
    ...
    - Optional: if there is time, make a scatter plot of `score` vs `bty_avg` with the points colored by gender and overlay the fitted lines from the main effects model. 
...
Slope: For an increase of 1 in the bty_avg score, we can expect an increase of 0.074 in the evaluation score. 

Intercept: The score for men is 0.172 higher than for women (regardless of `bty_avg`).

### Exercise 4

Repeat exercise 3 for an interaction effects linear model, `evals_int`, which predicts course evaluation `score` from  average beauty rating (`bty_avg`), `gender` and their interaction. 
...
```{r}
glance(evals_slr) %>% select(adj.r.squared)
glance(evals_main) %>% select(adj.r.squared)
glance(evals_int) %>% select(adj.r.squared)
```
