...
What does each row represent? A recorded accident, the registration number of a car, the location of an accident, an insurance claim?

### Warm up 2

The variable `veh_count` describes the number of cars involved in the collision. How many accidents involved 2 or more cars?


### Warm up 3

The variable `addr_type` describes the location of the accident. Where do most accidents occur?  Create a frequency table of `addr_type` in descending order of frequency.
...
Create a frequency table of each variable to examine their distributions. Go ahead and write code to do this.


As we can see, the `light` variable has three categories of darkness, two of which are quite sparse. Perhaps we can combine all three into one category and call it "Dark"?
...
Now let us examine whether light conditions influence whether or not a parked car is hit.

### Exercise 3

Create a contingency table of the frequencies for `light_cat` and `hit_parked_car` by filling in the blanks. If there is no association between these variables, what % of cars should be hit regardless of light conditions?
...
```{r eval = F}
    ...
    accidents %>% 
       tabyl( ___, ___) %>% 
       adorn_totals(where = ___ ) %>%
       adorn_percentages() %>%
       adorn_pct_formatting(digits=___) %>%
       adorn_ns() %>%
       adorn_title()
```
...
Make a segmented bar plot to visualize the contingency table from exercise 3 by filling in the blanks in the code below.
...
Fill in the blanks to create a mosaicplot to visualize the strength of the associations seen in the contingency table. Write a couple of sentences to describe what you learn.
...
- Make a frequency distribution of each variable separately.
...
- Create a contingency table of the frequencies for `addr_type` and `severity_desc`. If there is no association between these variables, what % of collisions should be injury collisions regardless of address type?

    *Hint* think about whether you want `addr_type` on the rows or the column. 
    
  
- Make a segmented bar plot to visualize the contingency table you just created.


- Make a mosaicplot to visualize the contingency table from you just created
